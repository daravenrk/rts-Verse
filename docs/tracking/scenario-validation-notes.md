...
- Result: Pass
- Notes:
  - F-75 uses deterministic mixed-loss pressure (`5 produced + 1 baseline squad`) because produced-only sext-loss is impossible with current slot-B cap composition.
  - F-75 validated three adaptive-profile cycles under deterministic sext-loss mixed pressure (`before=10`, `after_loss=4`, `after_recovery=10`).
  - F-75 aggregate checks passed: `cycles_pass=true`, `profile_coverage_pass=true`, `sext_loss_coverage_pass=true`, `cap_recovery_bound_coverage_pass=true`, `cap_hold_pass=true`, `diversity_pass=true`, `max_units_seen=10`, `diversity=3`.
  - Regression checks for F-74, F-73, F-72, F-71, F-70, F-69, F-68, F-67, F-66, F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.

## 2026-06-14 Enemy Adaptive-Jitter Quint-Loss Readiness Slice Validation

- Flow: `Adaptive-profile jitter recovery cycles with deterministic quint-loss pressure and cap safety`
- Scenario: `scenes/core/FirstDuelMap.tscn`
- Validation command:
...
- Result: Pass
- Notes:
  - F-74 validated four adaptive-profile cycles under deterministic five-unit loss pressure (`before=10`, `after_loss=5`, `after_recovery=10`).
  - F-74 aggregate checks passed: `cycles_pass=true`, `profile_coverage_pass=true`, `quint_loss_coverage_pass=true`, `cap_recovery_bound_coverage_pass=true`, `cap_hold_pass=true`, `diversity_pass=true`, `max_units_seen=10`, `diversity=3`.
  - Regression checks for F-73, F-72, F-71, F-70, F-69, F-68, F-67, F-66, F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.
...
- Result: Pass
- Notes:
  - F-72 validated six adaptive-profile cycles under deterministic three-unit loss pressure (`before=10`, `after_loss=7`, `after_recovery=10`).
  - F-72 aggregate checks passed: `cycles_pass=true`, `profile_coverage_pass=true`, `triple_loss_coverage_pass=true`, `cap_recovery_bound_coverage_pass=true`, `cap_hold_pass=true`, `diversity_pass=true`, `max_units_seen=10`, `diversity=3`.
  - Regression checks for F-71, F-70, F-69, F-68, F-67, F-66, F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.

## 2026-06-14 Enemy Adaptive-Jitter Dual-Loss Readiness Slice Validation

- Flow: `Adaptive-profile jitter recovery cycles with deterministic dual-loss pressure and cap safety`
- Scenario: `scenes/core/FirstDuelMap.tscn`
- Validation command:
...
- Result: Pass
- Notes:
  - F-71 validated seven adaptive-profile cycles under deterministic two-unit loss pressure (`before=10`, `after_loss=8`, `after_recovery=10`).
  - F-71 aggregate checks passed: `cycles_pass=true`, `profile_coverage_pass=true`, `dual_loss_coverage_pass=true`, `cap_hold_pass=true`, `diversity_pass=true`, `max_units_seen=10`, `diversity=3`.
  - Regression checks for F-70, F-69, F-68, F-67, F-66, F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.
...
- Result: Pass
- Notes:
  - F-69 validated six adaptive-profile recovery cycles with bounded recovery timing (`recovery_step=58,65,64` pattern by profile).
  - F-69 aggregate checks passed: `cycles_pass=true`, `profile_coverage_pass=true`, `cap_hold_pass=true`, `diversity_pass=true`, `max_units_seen=10`, `diversity=3`.
  - Regression checks for F-68, F-67, F-66, F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.

## 2026-06-14 Enemy Long-Horizon Timer-Jitter Recovery Readiness Slice Validation

- Flow: `Extended jittered timer-driven recovery cycles with timing bounds and diversity safety`
- Scenario: `scenes/core/FirstDuelMap.tscn`
- Validation command:
  - `/Applications/Godot.app/Contents/MacOS/godot --headless --quit --path . res://scenes/core/FirstDuelMap.tscn -- --duel-test-f68-enemy-timer-jitter-long-horizon --duel-test-f67-enemy-timer-jitter-recovery --duel-test-f66-enemy-timer-cycle-stress --duel-test-f65-enemy-timer-recovery --duel-test-f64-enemy-recovery-stress --duel-test-f63-enemy-cap-recovery --duel-test-f62-enemy-production-horizon --duel-test-f61-enemy-ai --duel-test-f38-production --duel-test-f37-combat --duel-test-f32-interaction --duel-test-f60-drag-select`
- Result: Pass
- Notes:
  - F-68 validated five long-horizon jittered recovery cycles with bounded recovery timing (`recovery_step=62` each cycle).
  - F-68 aggregate checks passed: `cycles_pass=true`, `jitter_path_pass=true`, `cap_hold_pass=true`, `diversity_pass=true`, `max_units_seen=10`, `diversity=3`.
  - Regression checks for F-67, F-66, F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.
...
- Scenario: `scenes/core/FirstDuelMap.tscn`
- Validation command:
  - `/Applications/Godot.app/Contents/MacOS/godot --headless --quit --path . res://scenes/core/FirstDuelMap.tscn -- --duel-test-f66-enemy-timer-cycle-stress --duel-test-f65-enemy-timer-recovery --duel-test-f64-enemy-recovery-stress --duel-test-f63-enemy-cap-recovery --duel-test-f62-enemy-production-horizon --duel-test-f61-enemy-ai --duel-test-f38-production --duel-test-f37-combat --duel-test-f32-interaction --duel-test-f60-drag-select`
- Result: Pass
- Notes:
  - F-66 validated three timer-driven recovery cycles with per-cycle loss, growth, and return-to-cap continuity (`recovery_step=55` each cycle).
  - F-66 aggregate checks passed: `cycles_pass=true`, `timer_path_pass=true`, `cap_hold_pass=true`, `max_units_seen=10`.
  - Regression checks for F-65, F-64, F-63, F-62, F-61, F-38, F-37, F-32, and F-60 remained pass with no script errors.
...
- Result: Pass (media lane `READY`)
- Notes:
  - Capture run generated required screenshots: `stage0-shot-01-opening-expansion.png`, `stage0-shot-02-contested-objective.png`, `stage0-shot-03-faction-asymmetry.png`.
...
- Command: `RTS_TEST_LOG_DIR=/tmp/rts-verse-parse-fix-final scripts/tests/run_headless_suite.sh`
- Result: Pass; 38 cases passed and zero failed on Godot 4.5.1 after a cold editor-parser correction. Logs: `/tmp/rts-verse-parse-fix-final/run.ODeUpb`.
